var escScheduler = (function () {
    ...
    var getSchedulerTimelineContent = function (date, $eventsContainer) {
        var departmentId = $("#DepartmentId").val();
        if (departmentId > 0) {
            $eventsContainer.html("<div class='text-center'>Loading events..</div>");

            var showShiftTimeline = function(response) {
                $eventsContainer.html(response);
                $("#divEntityResult #linkSearch").trigger("click");
            };
            Framework.Utility.ExecuteAjaxRequest(defaults.urlGetEvents, { DepartmentId: departmentId, RequestDate: date }, showShiftTimeline);
        } else {
            $eventsContainer.html("<div class='text-center'>Please choose a department..</div>");
            $("#DepartmentId_chosen").trigger("mousedown");
        }
    }
    ...
    var importCalendarContent = function (event, $eventsContainer) {
        switch (event.data.schedulerId) {
            ...
            case "divEntityScheduler":
                return getSchedulerTimelineContent(event.data.date, $eventsContainer);
        default:
        };
