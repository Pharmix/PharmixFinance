...
namespace Pharmix.Web.Services
{
    public class UserService : IUserService
    {
        ...
        {
            if (userName.Equals(PharmixStaticHelper.SuperAdminUsername, StringComparison.CurrentCultureIgnoreCase))
            {
                return new List<string>() { PharmixStaticHelper.SuperAdminRoleName };
            }
        ...
        private void UpdateUserTempIdCache()
        {
            ...
            var userTempDictionary = _cacheService.Get<Dictionary<int, string>>(_userTempCache);
            if (userTempDictionary == null)
                userTempDictionary = new Dictionary<int, string>();

            var cachedUserIdList = userTempDictionary.Select(x => x.Value).ToList();
            var count = (cachedUserIdList.Count() == null || cachedUserIdList.Count() == 0) ? 1 : cachedUserIdList.Count() + 1;
            var users = _repository.GetContext().Users.ToList().Where(x => !cachedUserIdList.Contains(x.Id));
            int i = count;
            if (users != null)
                foreach (var user in users)
                {
                    userTempDictionary.Add(i++, user.Id);
                }
            _cacheService.Set(_userTempCache, userTempDictionary);
        }
        ...
        public UserDepartmentsViewModel CreateUserDepartmentsViewModel(int id, int trustId)
        {
            ...
            var model = new UserDepartmentsViewModel(){ UserId = userId };
            ...
            model.DepartmentList = _repository.GetAll<Department>().Where(p => p.TrustId == trustId && !p.IsArchived).Select(dept =>
                new SelectListItem {Value = dept.DepartmentId.ToString(), Text = dept.DepartmentName, Selected = userDepts.Any(ud => ud == dept.DepartmentId) }).ToList();
        ...
        public BaseResultViewModel<string> UpdateUserDepartments(UserDepartmentsViewModel model, string userName)
        {
            ...
            {
                return new BaseResultViewModel<string>() { IsSuccess = false, Message = "User departments update failed." };
            }
            
            return new BaseResultViewModel<string>(){ IsSuccess = true, Message = "User departments updated successfully."};
        }
    }
